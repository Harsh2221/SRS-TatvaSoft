<?php 
$base_url="https://localhost/tatvasoft/Helperland_MVC/";
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Homepage</title>
    <link rel="stylesheet" href="./assets/css/bootstrap.min.css" />
    <link rel="stylesheet" href="./assets/css/homepage.css" />
  </head>

  <body>

    <!-- landing page -->
    <section id="page-header" class="site">
      
      <div id="homepage" class="container-fluid sitebanner">
        <nav class="navbar navbar-expand-lg navbar-light">
          <div class="container-fluid">
            <a class="navbar-brand" href="homepage.php"
              ><img src="./assets/assets/logo-large.png" alt="" />
            </a>
            <button
              class="navbar-toggler"
              type="button"
              data-bs-toggle="collapse"
              data-bs-target="#navbarSupportedContent"
              aria-controls="navbarSupportedContent"
              aria-expanded="false"
              aria-label="Toggle navigation"
            >
              <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
              <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                <li class="nav-item l-1">
                  <a class="nav-link active" aria-current="page" href="book_services.php"
                    >Book a Cleaner</a
                  >
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="price.php">Prices</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="#">Our Guarantee</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="#">Blog</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="contact.php">Contact us</a>
                </li>
                <li class="nav-item l-2">
                  <a class="nav-link" href="#" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#login_modal">Login</a>
                </li>
                <li class="nav-item l-3">
                  <a class="nav-link" href="service_provider_become_pro.php">Become a Helper</a>
                </li>
                <li class="nav-item dropdown">
                  <a
                    class="nav-link dropdown-toggle"
                    href="#"
                    id="navbarDropdown"
                    role="button"
                    data-bs-toggle="dropdown"
                  >
                    <img src="./assets/assets/ic-flag.png" alt="#" />
                  </a>
                  <ul
                    class="dropdown-menu"
                    aria-labelledby="navbarDropdownMenuLink"
                  >
                    <li>
                      <a class="dropdown-item" href="#"
                        ><img src="./assets/assets/ic-flag.png" alt="#"
                      /></a>
                    </li>
                  </ul>
                </li>
              </ul>
            </div>
          </div>
        </nav>

        <div class="modal fade" id="login_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="login-modal" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">Login to your account</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                <form method="POST" class="form" action=<?= $base_url."./?controller=main_&function=login"?>>
                  <div class="col">
                    <input type="email" class="form-control" placeholder="Email" aria-label="email"
                    name="user_email" value="<?php if (isset($_COOKIE['email_cookie'])) {  echo $_COOKIE['email_cookie']; } ?>">
                    
                  </div>
                  <div class="col">
                    <input type="password" class="form-control" placeholder="password" aria-label="password"
                    name="pass" value="<?php if (isset($_COOKIE['password_cookie'])) {  echo $_COOKIE['password_cookie']; } ?>">
                  </div>
                  
                  <div class="form-check">
                  <?php if (isset($_COOKIE)) { ?>
                    <input class="form-check-input" type="checkbox" checked id="gridCheck"
                    name="remember">
                  <?php }
                    if (!isset($_COOKIE['emailcookie'])) { ?>
                    <input class="form-check-input" type="checkbox" id="gridCheck"
                    name="remember">
                    <?php } ?>
                    <label class="form-check-label" for="gridCheck">
                      Remember Me
                    </label>
                  </div>
                  <div class="login">
                    <button type="submit" id="login">
                      Login
                    </button>
                  </div>
                  <div class="model-footer">
                    <a href="#" data-bs-toggle="modal" data-bs-target="#forgot">Forgot Password ?</a>
                    <p>Don't have an account? <span><a href="registration_page.php">Create an account</a></span></p>

                  </div>

                </form>
              </div>
              
            </div>
          </div>
        </div>

        <div class="modal fade" id="forgot" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="staticBackdropLabel">Forgot password</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body">
                <form method="POST" class="form" action=<?= $base_url."./?controller=main_&function=forgotPassword"?>>
                  <div class="col">
                    <input type="email" class="form-control" placeholder="Email" aria-label="email"
                    name="email">
                  </div>
                  <div class="send">
                    <button type="submit" id="send">
                      Send
                    </button>
                  </div>
                  <div class="F-login">
                    <a href="#" data-bs-toggle="modal" data-bs-target="#login_modal">Login now</a>
                  </div>
                </form>
              </div>
              
            </div>
          </div>
        </div>



        <div class="hero">
          <h2>Lorem ipsum text</h2>
          <p>
            <span><img src="./assets/assets/ic-check.png" alt="" /></span> Lorem ipsum
            dolor sit amet, consectetur adipiscing
          </p>
          <p>
            <span><img src="./assets/assets/ic-check.png" alt="" /></span> Lorem ipsum
            dolor sit amet, consectetur adipiscing
          </p>
          <p>
            <span><img src="./assets/assets/ic-check.png" alt="" /></span> Lorem ipsum
            dolor sit amet, consectetur adipiscing
          </p>
        </div>

        <div class="row">
          <div class="col-lg-12 col-md-6 col-sm-6 text-center boo-btn">
            <button class="book-button">Let's Book a Cleaner</button>
          </div>
        </div>

        <div class="step-wrapper">
          <div class="row justify-content-center flex-wrap">
            <div class="col-lg-2 col-sm-6 process-item-1 full-width">
              <div class="step">
                <span>
                  <img src="./assets/assets/step-1.png" alt="Enter Postcode" />
                </span>
                <p>Enter your postcode</p>
              </div>
            </div>
            <div class="col-lg-1 col-sm-6 process-item-1 full-width">
              <div class="step">
                <span>
                  <img src="./assets/assets/step-arrow-1.png" alt="Enter Postcode" />
                </span>
              </div>
            </div>

            <div class="col-lg-2 col-sm-6 process-item-2 full-width">
              <div class="step">
                <span>
                  <img src="./assets/assets/step-2.png" alt="Select Plan" />
                </span>
                <p>Select your plan</p>
              </div>
            </div>
            <div class="col-lg-1 col-sm-6 process-item-1 full-width">
              <div class="step">
                <span>
                  <img
                    src="./assets/assets/step-arrow-1-copy.png"
                    alt="Enter Postcode"
                  />
                </span>
              </div>
            </div>
            <div class="col-lg-2 col-sm-6 process-item-3 full-width">
              <div class="step">
                <span>
                  <img src="./assets/assets/step-3.png" alt="Pay Securely" />
                </span>
                <p>Pay securely online</p>
              </div>
            </div>
            <div class="col-lg-1 col-sm-6 process-item-1 full-width">
              <div class="step">
                <span>
                  <img src="./assets/assets/step-arrow-1.png" alt="Enter Postcode" />
                </span>
              </div>
            </div>
            <div class="col-lg-2 col-sm-6 process-item-4 full-width">
              <div class="step">
                <span>
                  <img src="./assets/assets/step-4.png" alt="Amazing service" />
                </span>
                <p>Enjoy amazing service</p>
              </div>
            </div>
          </div>
        </div>

        <div class="down-arrow">
          <a href="#whyhelperland"><img src="./assets/assets/group-18_5.png" alt="#" class="arrow-img" /></a>
          
        </div>
      </div>
    </section>
    <!-- //landing page  -->


    <!-- why-helperland section  -->
    <div class="container why-helperland" id="whyhelperland">
      <div class="row">
        <div class="col-lg-12 text-center text-title">
          <h1>Why Helperland</h1>
        </div>
        <div class="col-lg-4 col-md-6">
          <div class="services">
            <div class="service-img text-center">
              <img src="./assets/assets/professionals (1).png" alt="s-1" />
            </div>
            <div class="card-body text-center">
              <h4 class="card-title">Experience & Vetted Professionals</h4>
              <p class="card-text" style="margin-top: 22px">
                dominate the industry in scale and scope with an adaptable,
                extensive network that consistently delivers exceptional
                results.
              </p>
            </div>
          </div>
        </div>

        <div class="col-lg-4 col-md-6">
          <div class="services">
            <div class="service-img text-center">
              <img src="./assets/assets/group-23.png" alt="s-1" />
            </div>
            <div class="card-body text-center">
              <h4 class="card-title">Secure Online Payment</h4>
              <p class="card-text">
                dominate the industry in scale and scope with an adaptable,
                extensive network that consistently delivers
              </p>
            </div>
          </div>
        </div>

        <div class="col-lg-4 col-md-6">
          <div class="services">
            <div class="service-img text-center">
              <img src="./assets/assets/group-24.png" alt="s-1" />
            </div>
            <div class="card-body text-center">
              <h4 class="card-title">Dedicated Customer Service</h4>
              <p class="card-text">
                to our customers and are guided in all we do by their needs. The
                team is always happy to support you and offer all the
                information.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- //why-helperland section  -->


    <!-- blog section  -->
    <section class="blog-section">
      <div class="container">
          <div class="blog-description-block">
              <div class="row align-items-center">
                  <div class="col-lg-7 col-md-6">
                      <h2 class="h3">Lorem ipsum dolor sit amet, consectetur</h2>
                      <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec nisi sapien, suscipit ut accumsan vitae, pulvinar ac libero.</p>
                      <p>Aliquam erat volutpat. Nullam quis ex odio. Nam bibendum cursus purus, vel efficitur urna finibus vitae. Nullam finibus aliquet pharetra. Morbi in sem dolor. Integer pretium hendrerit ante quis vehicula.</p>
                      <p>Mauris consequat ornare enim, sed lobortis quam ultrices sed.</p>
                  </div>
                  <div class="col-lg-5 col-md-6">
                      <div class="image-wrapper">
                          <img src="./assets/assets/group-36.png"Blog description">
                      </div>
                  </div>
              </div>
          </div>
          <div class="our-blog-block">
              <h2 class="text-center">Our Blog</h2>
              <div class="row blog-wrapper">
                  <div class="col-lg-4 col-sm-6 blog-detail-outer">
                      <div class="blog-detail">
                          <div class="blog-image">
                              <img src="./assets/assets/group-28.png" alt="blog-image">
                          </div>
                          <div class="blog-content">
                              <h3>Lorem ipsum dolor sit amet</h3>
                              <span>January 28, 2019</span>
                              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed fermentum metus pulvinar aliquet.</p>
                              <a href="#" title="Read More" class="arrow-link">Read More<em><img src="./assets/assets/shape-2.png" alt="right arrow"></em></a>
                          </div>
                      </div>
                  </div>
                  <div class="col-lg-4 col-sm-6 blog-detail-outer">
                      <div class="blog-detail">
                          <div class="blog-image">
                              <img src="./assets/assets/group-29.png" alt="blog-image">
                          </div>
                          <div class="blog-content">
                              <h3>Lorem ipsum dolor sit amet</h3>
                              <span>January 28, 2019</span>
                              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed fermentum metus pulvinar aliquet.</p>
                              <a href="#" title="Read More" class="arrow-link">Read More<em><img src="./assets/assets/shape-2.png" alt="right arrow"></em></a>
                          </div>
                      </div>
                  </div>
                  <div class="col-lg-4 col-sm-6 blog-detail-outer">
                      <div class="blog-detail">
                          <div class="blog-image">
                              <img src="./assets/assets/group-30.png" alt="blog-image">
                          </div>
                          <div class="blog-content">
                              <h3>Lorem ipsum dolor sit amet</h3>
                              <span>January 28, 2019</span>
                              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed fermentum metus pulvinar aliquet.</p>
                              <a href="#" title="Read More" class="arrow-link">Read More<em><img src="./assets/assets/shape-2.png" alt="right arrow"></em></a>
                          </div>
                      </div>
                  </div>
              </div>
          </div>
      </div>
  </section>
    <!-- //blog section  -->

    <!-- customer section  -->
    <div class="customer">
      <div class="row">
        <div class="col-lg-12 text-center text-title">
          <h1>What Our Customers Say</h1>
        </div>
        <div class="customer-divs">
          <div class="row">
            <div class="col-lg-4 col-md-3">
              <div class="card custm">
                <div class="card-body customer-item">
                  <h4 class="name-title">
                    <span>
                      <img src="./assets/assets/group-31.png" class="cardimg" alt="..."
                    /></span>
                    Lary Watson
                  </h4>
                  <p class="sub-name">Manchester</p>
                  <img src="./assets/assets/message.png" alt="" class="msg-img" />

                  <p class="card-text" style="color: #565656">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed
                    fermentum metus pulvinar aliquet consequat. Praesent nec
                    malesuada nibh. <br />
                    <br />
                    Nullam et metus congue, auctor augue sit amet, consectetur
                    tortor.
                  </p>
                  <a href="#" class="btn"
                    >Read the Post
                    <span><img src="./assets/assets/shape-2.png" alt="#" /></span
                  ></a>
                </div>
              </div>
            </div>
            <div class="col-lg-4 col-md-3">
              <div class="card custm">
                <div class="card-body customer-item">
                  <h4 class="name-title">
                    <span>
                      <img src="./assets/assets/group-32.png" class="cardimg" alt="..."
                    /></span>
                    Lary Watson
                  </h4>
                  <p class="sub-name">Manchester</p>
                  <img src="./assets/assets/message.png" alt="" class="msg-img" />

                  <p class="card-text" style="color: #565656">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed
                    fermentum metus pulvinar aliquet consequat. Praesent nec
                    malesuada nibh. <br />
                    <br />
                    Nullam et metus congue, auctor augue sit amet, consectetur
                    tortor.
                  </p>
                  <a href="#" class="btn"
                    >Read the Post
                    <span><img src="./assets/assets/shape-2.png" alt="#" /></span
                  ></a>
                </div>
              </div>
            </div>
            <div class="col-lg-4 col-md-3">
              <div class="card custm">
                <div class="card-body customer-item">
                  <h4 class="name-title">
                    <span>
                      <img src="./assets/assets/group-33.png" class="cardimg" alt="..."
                    /></span>
                    Lary Watson
                  </h4>
                  <p class="sub-name">Manchester</p>
                  <img src="./assets/assets/message.png" alt="" class="msg-img" />

                  <p class="card-text" style="color: #565656">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed
                    fermentum metus pulvinar aliquet consequat. Praesent nec
                    malesuada nibh. <br />
                    <br />
                    Nullam et metus congue, auctor augue sit amet, consectetur
                    tortor.
                  </p>
                  <a href="#" class="btn"
                    >Read the Post
                    <span><img src="./assets/assets/shape-2.png" alt="#" /></span
                  ></a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- //customer section  -->

    <!-- get our newsletter section  -->
    <div class="below-customer">
      <div class="email-div">
        <h5>GET OUR NEWSLETTER</h5>
        <form>
          <input
            type="email"
            class="email-box"
            id=""
            placeholder="YOUR EMAIL"
          />

          <button type="submit" class="submitbtn">Submit</button>
        </form>
        
      </div>
    </div>
    <!-- //get our newsletter section  -->

    <!-- Footer -->
    <footer class="page-footer">
        <div class="row">
          <div class="col-lg-2 footer-img">
            <img src="./assets/assets/footer-logo.png" alt="" />
          </div>
          <div class="col-lg-7 col-md-3 footer-links">
            <a href="" class="f-links"><span>HOME</span></a>
            <a href="about.php" class="f-links"><span>ABOUT</span></a>
            <a href="" class="f-links"><span>TESTIMONIALS</span></a>
            <a href="faqs.php" class="f-links"><span>FAQS</span></a>
            <a href="" class="f-links"><span>INSURANCE POLICY</span></a>
            <a href="" class="f-links"><span>IMPRESSUM</span></a>
          </div>
          <div class="col-lg-2 col-md-3 footer-social">
            <a href="" class="f-social"
              ><img src="./assets/assets/ic-facebook.png" alt=""
            /></a>
            <a href="" class="f-social"
              ><img src="./assets/assets/ic-instagram.png" alt=""
            /></a>
          </div>
        </div>
      </footer>
      <!-- //Footer -->

    <!-- copyright section  -->
    <div class="copyright">
      <div class="row">
        <div class="col-lg-12 col-md-4">
          <p style="color: #f1f1f1; font-size: 14px">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut feugiat
            nunc libero, ac malesuada ligula aliquam ac.
            <a href="">Privacy Policy</a>
            <button class="ok-btn">OK!</button>
          </p>
        </div>
      </div>
    </div>
    <div class="top">
      <a href="#homepage"
        ><img src="./assets/assets/down-arrow - Copy.png" alt="" class="upside-img"
      /></a>
    </div>
    <!-- //copyright section  -->

    <script src="./assets/js/bootstrap.min.js"></script>
    <script src="./assets/js/jquery.js"></script>
   
    <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
    
    <?php
    if(isset($_SESSION['msg'])){

    ?>
    <script>
    swal({
      title: "<?php echo $_SESSION['msg'] ?>",
      icon: "<?php echo $_SESSION['icon'] ?>",
      button: "OK",
    });

    </script>
    <?php
    unset($_SESSION['msg']);
    }
    ?>
    
  </body>
</html>
